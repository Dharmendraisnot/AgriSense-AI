...
class BaseAgent(ABC):
    ...
    def _get_model(self) -> Any:
        ...
        if self._model is None:
            # Read settings fresh so Streamlit Cloud secrets are always picked up
    ...
    def generate(self, prompt: str) -> str:
        ...
        """
        # Read settings fresh at call time so secrets injected by app.py are visible
        settings = get_settings()
        if not settings.watsonx_api_key:
            logger.warning("watsonx_api_key not set – returning placeholder response.")
            return (
                "[IBM Granite response – configure WATSONX_API_KEY in Streamlit secrets to enable]\n\n"
