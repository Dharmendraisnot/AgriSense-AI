...
def render() -> None:
    ...
    for msg in st.session_state["chat_history"]:
        if msg["role"] == "user":
            ...
            # Using clean block elements with hardcoded styles to prevent override
            st.markdown(
                f"""
                <div class="agent-bubble" style="background-color: #f0f2f6 !important; padding: 15px; border-radius: 10px; margin-bottom: 10px;">
                    <h4 style="color: #111111 !important; margin: 0 0 8px 0; font-family: sans-serif; display: flex; align-items: center; gap: 5px;">
                        🌾 {agent}:
                    </h4>
                    <p style="color: #111111 !important; margin: 0 0 10px 0; font-family: sans-serif; line-height: 1.5; font-size: 1rem;">
                        {msg["content"]}
                    </p>
                    {f'<p style="color: #555555 !important; font-size: 0.85em; margin: 0; font-family: sans-serif;">Sources: {sources}</p>' if sources else ""}
                </div>
                """,
                unsafe_allow_html=True,
            )
