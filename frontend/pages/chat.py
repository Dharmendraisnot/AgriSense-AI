...
def render() -> None:
    ...
    for msg in st.session_state["chat_history"]:
        if msg["role"] == "user":
            ...
            # Added style="color: #111111;" to the main text container to make it visible
            st.markdown(
                f'<div class="agent-bubble" style="color: #111111 !important;">'
                f'<span style="color: #111111; font-weight: bold;">🌾 {agent}:</span><br>'
                f'<span style="color: #222222;">{msg["content"]}</span>'
                + (f'<br><small style="color:#555555; font-size: 0.85em;">Sources: {sources}</small>' if sources else "")
                + "</div>",
                unsafe_allow_html=True,
            )
